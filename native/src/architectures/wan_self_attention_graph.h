...
inline neural_graph::Graph self_attention_graph(int64_t b, int64_t s, DType execution,
    ...
    require(config.heads > 0 && config.dim > 0 && config.dim % config.heads == 0, "Wan head config mismatch");
    const int64_t width = config.dim, head_width = width / config.heads;
    const std::vector<int64_t> hidden{b,s,width}, part{b,1,width}, heads{b,s,config.heads,head_width};
    ng::Description d; d.schema = ng::schema_v1; d.execution_dtype = execution;
    d.values = {{hidden,hidden_dtype},{{b,6,width}},{{1,s,1,head_width}},{{1,s,1,head_width}},
        {{1,6,width}},{{width,width},execution},{{width},execution},
        {{width,width},execution},{{width},execution},{{width,width},execution},{{width},execution},
        {{width}},{{width}},{{width,width},execution},{{width},execution},{{1}}};
    ...
    auto combined = node(ng::Add{4,1},{b,6,width});
    ...
    auto norm = node(ng::LayerNorm{0,{},{},config.epsilon},hidden,hidden_dtype);
    ...
    auto q_norm = node(ng::RmsNorm{q,11,2,config.epsilon},hidden);
    auto k_norm = node(ng::RmsNorm{k,12,2,config.epsilon},hidden);
    ...
    auto attention = node(ng::Attention{q_operand,k_operand,v_heads,1.0f/std::sqrt(static_cast<float>(head_width))},heads,execution);
...
inline std::vector<Tensor> self_attention(Backend& backend, const PrecisionPolicy& policy,
    ...
    auto graph = self_attention_graph(hidden.dim(0),hidden.dim(1),backend.execution_dtype(),hidden.dtype(),diagnostics,config);
