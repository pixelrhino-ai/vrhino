...
int main(int argc,char** argv) {
    try {
        ...
        if(argc!=5 && !(argc==6 && std::string(argv[5])=="--product")) {std::cerr<<"usage: wan-family-convert MODEL_ROOT SEMANTIC_SOURCE_ROOT SPEC OUTPUT_DIR [--product]\n"
            <<"       wan-family-convert verify VRM\n";return 2;}
        uint64_t work=0;auto last=std::chrono::steady_clock::now();
        auto progress=[&](uint64_t n){
            work+=n;auto now=std::chrono::steady_clock::now();
            if(now-last>std::chrono::seconds(15)){std::cerr<<"streamed_work_bytes="<<work<<'\n';last=now;}
        };
        auto result=argc==6
            ? vrhino::product::convert_wan_family_package(argv[1],argv[2],argv[3],argv[4],progress,
